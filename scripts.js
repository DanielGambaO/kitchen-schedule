...
function fillSelectWithNames(data, selectId, section) {
    ...
    const employeesInSection = data.filter((employee) => employee.section === section)
    ...
    employeesInSection.forEach((employee) => {
        const option = $("<option>")
            .val(`${employee.first_name} ${employee.last_name}`)
    ...
    data.forEach(function (employee) {
        ...
        if (dropdownIndex !== -1) {
            ...
            var employeeDiv = $('<a>', {
                class: 'dItem',
                id: 'file',
                href: 'schedule.html?section=' + section + '&fullName=' + employee.first_name + ' ' + employee.last_name,
                text: employee.first_name + ' ' + employee.last_name
            });
...
// Function to fill select with available and current hours
function fillHoursSelects(data, selectAvailableId, selectCurrentId, selectedName, selectedDay) {
    // Clear existing options
    selectAvailableId.empty();
    selectCurrentId.empty();

    // Find the selected employee
    const selectedEmployee = data.find((employee) => `${employee.first_name} ${employee.last_name}` === selectedName);

    if (selectedEmployee) {
        // Get the working hours for the selected day
        const workingHours = selectedEmployee.schedule[selectedDay] || [];

        // Fill the "available hours" select with hours not in the schedule
        for (let i = 7; i <= 16; i++) {
            const timeRange = `${i}-${i + 1}`;
            if (!workingHours.includes(timeRange)) {
                const option = $("<option>")
                    .val(timeRange)
                    .text(timeRange);
                selectAvailableId.append(option);
            }
        }

        // Fill the "current hours" select with hours in the schedule
        workingHours.forEach((timeRange) => {
            const option = $("<option>")
                .val(timeRange)
                .text(timeRange);
            selectCurrentId.append(option);
        });
    }
}
